import { ObjectDataParser } from '../parser/object-data-parser';
import { BinaryDataParser } from '../parser/binary-data-parser';
import { DragonBonesData } from '../model/dragonbones-data';
import { TextureAtlasData, TextureData } from '../model/texture-atlas-data';
import { DragonBones, BoneType, ConstraintType, DisplayType } from '../core/dragonbones';
import { DataParser } from '../parser/data-parser';
import { ArmatureData, SlotData } from '../model/armature-data';
import { Armature } from '../armature/armature';
import { BaseObject } from '../core/base-object';
import { Bone } from '../armature/bone';
import { Surface } from '../armature/surface';
import { DisplayData, ArmatureDisplayData, ImageDisplayData, MeshDisplayData } from '../model/display-data';
...
import { AnimationData } from '../model/animation-data';
...
export abstract class BaseFactory {
  ...
  protected readonly _dragonBonesDataMap: SMap<DragonBonesData> = {};
  protected readonly _textureAtlasDataMap: SMap<Array<TextureAtlasData>> = {};
  ...
  public clear(disposeData: boolean = true): void {
    for (let k of Object.keys(this._dragonBonesDataMap)) {
      if (disposeData) {
    ...
    for (let k of Object.keys(this._textureAtlasDataMap)) {
      if (disposeData) {
  ...
  public replaceAnimation(armature: Armature, armatureData: ArmatureData, isOverride: boolean = true): boolean {
    ...
    if (isOverride) {
      ...
      const animations: SMap<AnimationData> = {};

      for (let k of Object.keys(rawAnimations)) {
        animations[k] = rawAnimations[k];
      }

      for (let k of Object.keys(armatureData.animations)) {
   ...
   * @private
   */
  public getAllDragonBonesData(): SMap<DragonBonesData> {
    return this._dragonBonesDataMap;
  }
  /**
   * @private
   */
  public getAllTextureAtlasData(): SMap<Array<TextureAtlasData>> {
  ...
  protected _buildSlots(dataPackage: BuildArmaturePackage, armature: Armature): void {
    ...
    const skinSlots: SMap<Array<DisplayData | null>> = {};
    for (let k of Object.keys(defaultSkin.displays)) {
      const displays = defaultSkin.getDisplays(k) as any;
      skinSlots[k] = displays;
    }

    if (currentSkin !== defaultSkin) {
      for (let k of Object.keys(currentSkin.displays)) {
  ...
  protected _buildConstraints(dataPackage: BuildArmaturePackage, armature: Armature): void {
    ...
    for (let k of Object.keys(constraints)) {
