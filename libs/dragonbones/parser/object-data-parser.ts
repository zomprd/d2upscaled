...
import { BoneData, ArmatureData, SlotData, SurfaceData } from '../model/armature-data';
import { DragonBonesData } from '../model/dragonbones-data';
import { GeometryData, MeshDisplayData, PathDisplayData, DisplayData, ImageDisplayData, ArmatureDisplayData, BoundingBoxDisplayData, WeightData } from '../model/display-data';
import { SkinData } from '../model/skin-data';
import { AnimationData, TimelineData, AnimationTimelineData } from '../model/animation-data';
import { Matrix } from '../geom/matrix';
import { Transform } from '../geom/transform';
import { Point } from '../geom/point';
import { ColorTransform } from '../geom/color-transform';
import { ActionData, UserData } from '../model/user-data';
...
import { CanvasData } from '../model/canvas-data';
import { ConstraintData, IKConstraintData, PathConstraintData } from '../model/constraint-data';
import { BoundingBoxData, RectangleBoundingBoxData, EllipseBoundingBoxData, PolygonBoundingBoxData } from '../model/bounding-box-data';
...
export class ObjectDataParser extends DataParser {
  ...
  private readonly _weightSlotPose: SMap<Array<number>> = {};
  private readonly _weightBonePoses: SMap<Array<number>> = {};
  private readonly _cacheBones: SMap<Array<BoneData>> = {};
  private readonly _slotChildActions: SMap<Array<ActionData>> = {};
  ...
  protected _parseArmature(rawData: any, scale: number): ArmatureData {
    ...
    for (let k of Object.keys(this._weightSlotPose)) {
      delete this._weightSlotPose[k];
    }
    for (let k of Object.keys(this._weightBonePoses)) {
      delete this._weightBonePoses[k];
    }
    for (let k of Object.keys(this._cacheBones)) {
      delete this._cacheBones[k];
    }
    for (let k of Object.keys(this._slotChildActions)) {
      ...
      for (const rawTimeline of rawTimelines) {
        ...
        switch (timelineType) {
          ...
          case TimelineType.SlotDeform: {
            this._geometry = null as any; //
            for (const skinName of Object.keys(this._armature.skins)) {
              const skin = this._armature.skins[skinName];
              for (const slontName of Object.keys(skin.displays)) {
  ...
  protected _parseSlotColorFrame(rawData: any, frameStart: number, frameCount: number): number {
    ...
    if (DataParser.VALUE in rawData || DataParser.COLOR in rawData) {
      ...
      for (let k of Object.keys(rawColor)) { // Detects the presence of color.
        // tslint:disable-next-line:no-unused-expression
        k;
